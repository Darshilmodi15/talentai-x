...
async def parse_node(state: PipelineState) -> PipelineState:
    ...
    for attempt in range(2):
        ...
        if any(fatal in err for err in state.get("errors", []) for fatal in ["Gemini quota exceeded", "Gemini model not found", "Gemini JSON format invalid"]):
...
async def finalize_node(state: PipelineState) -> PipelineState:
    ...
    logger.info(f"Total Gemini requests generated by one upload {state.get('job_id', 'unknown')}: {total_calls}")
    return state
